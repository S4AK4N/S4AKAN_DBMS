...
class Database {
    ...
    private fun displayResults(results: List<Row>, requestedColumns: List<String>, table: Table) {
        ...
        val actualColumns = if (requestedColumns == listOf("*")) {
        ...
        // データ行表示
        results.forEach { row ->
            val dataRow = actualColumns.indices.joinToString(" | ") { index ->
                val columnIndex = table.schema.columns.indexOfFirst { it.name == actualColumns[index] }
            ...
            val jsonData = buildString {
                ...
                tableEntries.forEachIndexed { index, (tableName, table) ->
                    ...
                    table.schema.columns.forEachIndexed { colIndex, column ->
                        append("        {\"name\": \"${column.name}\", \"type\": \"${column.type}\"}")
